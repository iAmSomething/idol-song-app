...
Railway runtime은 `backend/nixpacks.toml`을 통해 `npm ci -> npm run build -> npm run start` 순서를 강제한다. `backend/dist`는 gitignore 대상이므로, remote build 단계가 `npm run build`를 생략하면 production/preview 모두 `node ./dist/server.js`에서 즉시 실패한다.
...
- entity / alias / official link: `artist_profiles_seed.json`
- channel allowlist: `youtube_channel_allowlists.json`
- release baseline: `verified_release_history_mb.json`
- release detail / service / track / MV state: `release_detail_catalog.json`
- artwork: `release_artwork_catalog.json`
...
latest/recent blocker 코호트만 빠르게 재생성할 때는 아래처럼 scoped rebuild를 사용한다. 이 경로는 전체 canonical snapshot을 덮어쓰지 않고 target cohort row만 다시 계산한 뒤 coverage/report queue도 같은 scope로 갱신한다.
...
이 명령은 아래 primary snapshot을 source-of-truth로 사용하고, web runtime mirror는 secondary export로만 본다.
