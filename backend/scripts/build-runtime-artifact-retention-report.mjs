...
} from './lib/runtimeArtifactRetention.mjs';
...
const DEFAULT_JSON_REPORT_PATH = path.join(BACKEND_DIR, 'reports', 'runtime_artifact_retention_report.json');
const DEFAULT_MARKDOWN_REPORT_PATH = path.join(BACKEND_DIR, 'reports', 'runtime_artifact_retention_report.md');
...
async function main() {
  ...
  const duplicates = await collectRuntimeArtifactDuplicates(REPO_DIR);
  const report = buildRuntimeArtifactRetentionReport({ duplicates });
  const markdown = renderRuntimeArtifactRetentionMarkdown(report);
