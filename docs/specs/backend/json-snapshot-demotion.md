...
- `VITE_SEARCH_SOURCE`
- `VITE_ENTITY_DETAIL_SOURCE`
- `VITE_RELEASE_DETAIL_SOURCE`
- `VITE_CALENDAR_MONTH_SOURCE`
- `VITE_RADAR_SOURCE`

운영 원칙은 아래와 같다.

- backend-primary production을 열 때는 `VITE_API_BASE_URL`과 `VITE_PRIMARY_SURFACE_SOURCE=api`를 함께 설정한다.
- emergency rollback은 build 전체를 되돌리는 대신 per-surface env 또는 query override로 `json` fallback을 강제할 수 있어야 한다.
