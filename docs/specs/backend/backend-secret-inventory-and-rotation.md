...
| `VITE_RELEASE_DETAIL_SOURCE` | variable | repository variable 또는 `github-pages` environment variable | workflow reference 있음, current visible lists는 empty | `deploy-pages.yml` | `web release owner` | remaining surface-local cutover / rollback | release detail만 transitional JSON fallback으로 되돌릴 수 있게 유지 |
