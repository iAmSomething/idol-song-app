...
| `VITE_SEARCH_SOURCE` / `VITE_ENTITY_DETAIL_SOURCE` / `VITE_RELEASE_DETAIL_SOURCE` / `VITE_CALENDAR_MONTH_SOURCE` / `VITE_RADAR_SOURCE` | variable | repository variable 또는 `github-pages` environment variable | workflow reference 있음, current visible lists는 empty | `deploy-pages.yml` | `web release owner` | surface-local cutover / rollback | 문제 surface만 JSON fallback으로 되돌릴 수 있게 유지 |
