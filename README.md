...
- 매일 새 컴백 탐지 워크플로: https://github.com/iAmSomething/idol-song-app/actions/workflows/weekly-kpop-scan.yml
- release-day hourly verification 워크플로: https://github.com/iAmSomething/idol-song-app/actions/workflows/release-day-verification.yml
...
- 검색 테스트 전략: `docs/specs/web/search-test-strategy.md`
...
- 모바일 iOS 실제 설치/배포 경로: `mobile/README.md`의 `실제 iPhone 앱 설치 / 배포 경로` (`npm run ios` 기본)
...
- GitHub Actions가 매일 오전 9시 7분 KST 기준으로 새 upcoming/news signal을 스캔
- exact-date same-day release verification은 별도 hourly workflow가 담당
- explicit release time이 있으면 `release_time - 1h`, 없으면 `12:00 KST`부터 same-day release promotion을 확인
...
- Android production Firebase App Distribution:
